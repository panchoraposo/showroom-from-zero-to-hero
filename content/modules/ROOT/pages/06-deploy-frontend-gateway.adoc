...
NOTE: If the **MCP** indicator stays red or the **Health Check** shows a DNS/proxy error, the `customer-service-mcp` app is not deployed yet (or is still building). Go back to Step 2 and confirm ArgoCD has an Application named `{{user_name}}-customer-service-mcp` in namespace `{user_name}-neuralbank` and that it is **Synced/Healthy**.
